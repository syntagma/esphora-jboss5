...
public class FEParamGetPtosVenta implements Serializable{
	...
	public void ejecutarConsulta(String cuit) {
		...
		fePtoVentaResponse = port.feParamGetPtosVenta(Long.parseLong(cuit.trim()));
		
		// llama a la clase FEMensajeDeError para validar el mensaje de Error.
		FEMensajeDeError.ejecutarMensajeDeError(fePtoVentaResponse.getErrors(), fePtoVentaResponse.getEvents(), facesMessages);
